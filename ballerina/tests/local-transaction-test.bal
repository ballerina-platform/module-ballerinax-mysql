...
function testLocalTransaction() returns error? {
    ...
    retry<SQLDefaultRetryManager>(1) transaction {
        _ = check dbClient->execute(`Insert into Customers (firstName,lastName,registrationID,creditLimit,
...
function testTransactionRollbackWithCheck() returns error? {
    Client dbClient = check new(host, user, password, localTransactionDB, port);
    error? ignore = testTransactionRollbackWithCheckHelper(dbClient);
...
function testTransactionRollbackWithCheckHelper(Client dbClient) returns error? {
    ...
    retry<SQLDefaultRetryManager>(1) transaction {
        ...
        _ = check dbClient->execute(`Insert into Customers (firstName,lastName,registrationID,
                creditLimit,country) values ('James', 'Clerk', 210, 5000.75, 'USA')`);
        _ = check dbClient->execute(`Insert into Customers2 (firstName,lastName,registrationID,
                    creditLimit,country) values ('James', 'Clerk', 210, 5000.75, 'USA')`);
...
function testLocalTransactionRollbackWithGeneratedKeys() returns error? {
    Client dbClient = check new (host, user, password, localTransactionDB, port);
    error? ignore = testLocalTransactionRollbackWithGeneratedKeysHelper(dbClient);
    //check whether update action is performed
    int count = check getCount(dbClient, "615");
    check dbClient.close();
...
function testLocalTransactionRollbackWithGeneratedKeysHelper(Client dbClient) returns error? {
    ...
    retry<SQLDefaultRetryManager>(1) transaction {
        ...
        _ = check dbClient->execute(`Insert into Customers
         (firstName,lastName,registrationID,creditLimit,country) values ('James', 'Clerk', 615, 5000.75, 'USA')`);
        _ = check dbClient->execute(`Insert into Customers2
         (firstName,lastName,registrationID,creditLimit,country) values ('James', 'Clerk', 615, 5000.75, 'USA')`);
        check commit;
    }
}
...
function testTransactionErrorPanicHelper(Client dbClient) returns error? {
    ...
    retry<SQLDefaultRetryManager>(1) transaction {
        transInfo = transactions:info();
        _ = check dbClient->execute(`Insert into Customers (firstName,lastName,
...
function testTwoTransactions() returns error? {
    Client dbClient = check new (host, user, password, localTransactionDB, port);
     ...
     retry<SQLDefaultRetryManager>(1) transaction {
         ...
         _ = check dbClient->execute(`Insert into Customers (firstName,lastName,registrationID,creditLimit,
                                     country) values ('James', 'Clerk', 400, 5000.75, 'USA')`);
         check commit;
     }
